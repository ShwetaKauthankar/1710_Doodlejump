...
function preload()
{
  img = loadImage("assets/doodle.png");
  img1 = loadImage("assets/bat.png");
}
...
Doodler.prototype.draw = function(altitude) {
	...
	if (this.enemy) {
		...
		if (altitude - this.location.y < height) {
			...
			image(img1,this.location.x, altitude - this.location.y + height / 2, this.size);
		} else {
			this.onScreen = false;
		}
	} else {
		...
		image(img,this.location.x, height / 2, this.size);  
	}
